...
export default function CompressPdf() {
  ...
  async function handleCompress() {
    ...
    try {
      setProgress(0)
      setResultUrl(null)
      const res = await uploadFile('/compress-pdf', formData, setProgress)
      const blob = new Blob([res.data], { type: res.headers['content-type'] })
      const url = window.URL.createObjectURL(blob)
      setResultUrl(url)
      const cd = res.headers['content-disposition'] || ''
      const filename = cd.match(/filename="?(.*)"?/)?.[1] || (file.name.replace(/\.pdf$/i, '-compressed.pdf'))
  ...
  return (
    <div>
      ...
      <div className="mt-4 flex items-center space-x-3">
        <button onClick={handleCompress} className="px-4 py-2 bg-blue-600 text-white rounded">Compress</button>
